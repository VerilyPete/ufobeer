...
/**
 * Creates a mock environment with tracked batch statements and queued messages.
 */
function createFallbackMockEnv() {
  const batchStatements: Array<{ sql: string; args: unknown[] }> = [];
  const queuedMessages: Array<{ body: unknown }> = [];
  ...
  return { mockEnv, batchStatements, queuedMessages };
}
...
describe('handleFallbackBatch', () => {
  ...
  it('extracts ABV from original descriptions', async () => {
    ...
    const mockMessages = createDefaultMessages();
  ...
  it('handles empty message array', async () => {
    const { mockEnv } = createFallbackMockEnv();
